 ...
 */
typedef enum {
  PI_I2C_XFER_STOP     = 0<<0,   /*!< Generate a STOP bit at the end of the transfer. */
  PI_I2C_XFER_NO_STOP  = 1<<0,   /*!< Don't generate a STOP bit at the end of the transfer. */
  PI_I2C_XFER_START    = 0<<1,    /*!< Generate a START bit at the beginning of the transfer. */
  PI_I2C_XFER_NO_START = 1<<1    /*!< Don't generate a START bit at the beginning of the transfer. */
} pi_i2c_xfer_flags_e;
 ...
 *
 * \param device  A pointer to the structure describing the device.
 * \param cmd       The command which specifies which parameters of the driver to modify and for some of them also their values. The command must be one of those defined in pi_i2c_ioctl_e.
 ...
 * \param flags  Specify additional transfer behaviors like start and stop bits management.
 */
void pi_i2c_read(struct pi_device *device, uint8_t *rx_buff, int length, pi_i2c_xfer_flags_e flags);
 ...
 * \param flags  Specify additional transfer behaviors like start and stop bits management.
 */
void pi_i2c_write(struct pi_device *device, uint8_t *tx_data, int length, pi_i2c_xfer_flags_e flags);
