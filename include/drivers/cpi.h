 ...
 *
 * This function must be called before the CPI device can be used. It configures the specified
 * device that can then be used to refer to the opened device when calling other functions.
 *
 ...
 *
 * This function can be called to close an opened CPI device once it is not needed anymore in order to free
 ...
 * This function is synchronous and will block the caller until the specified amount of bytes
 * is received.
 *
 * \param device    A pointer to the structure describing the device.
 ...
 *
 * Queue a buffer that will receive samples from the CPI interface.
 * This function asynchronous and will not block the caller.
 ...
 * way to retrieve data from the CPI device. You should always make sure that at least 2 buffers are always queued,
 ...
 *
 * \param device    A pointer to the structure describing the device.
 * \param nb_frame_dropped    Number of frames to drop. Can be set to 0 to deactivate this feature.
 */
 ...
 * Each channel is multiplied by a coefficient. They are then summed together and shifted
 * right to obtain an 8 bit pixel.
 *
 * \param device    A pointer to the structure describing the device.
 ...
 * \param shift    The sum of all channels multiplied by their coefficient is shifted right by this value to fit an 8 bit pixel.
 */
